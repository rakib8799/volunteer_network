...
const RegisterVolunteers = () => {
    ...
    const [event,] = useState(EventContext);
    const {id} = useParams();
    useEffect(() => {
        const fake=fakeData.find(fd=>fd.id===Number(id))
        setLoggedInUser(fake);
    },[id, setLoggedInUser])
    ...
    return (
        <div className='container'>
            ...
            <form className='form'>
                ...
                <input type='text' className='form-control' name="text" defaultValue={event?.name} placeholder="Organize books at the library"/>             
                <Link to='/event'>
